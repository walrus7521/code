...
def outw(register, channel, word):
    ...
    #print "write: {:02X}, {:02X}".format(reg, word)
...
def scan():
    ...
    for addr in range(0,128):
        if i2c.detect(addr, 0x5A) == True:
            ...
            print "  => found device at addr {:02X}h".format(addr)
...
def sc700_slave_read(slave_addr, num_bytes):
    outb(SC750_THR, TX_CHANNEL, SC700_START)
    outb(SC750_THR, TX_CHANNEL, (slave_addr << 1))
    ...
    outb(SC750_THR, TX_CHANNEL, SC700_STOP)
    outb(SC750_THR, TX_CHANNEL, SC700_START)
    outb(SC750_THR, TX_CHANNEL, (slave_addr << 1) | 0x01)
...
def sc750_init():
    ...
    outb(SC750_LCR, TX_CHANNEL, 0x87) # enable special registers, and line control (8:N:1)
    # divisor = 1152d = 480h = 11.0592 MHz / 9600 baud
    outb(SC750_DLH, TX_CHANNEL, 0x04) # baud rate divisor (high byte)
    outb(SC750_DLL, TX_CHANNEL, 0x80) # baud rate divisor (low byte)
    outb(SC750_LCR, TX_CHANNEL, 0x07) # disable special registers
    outb(SC750_MCR, TX_CHANNEL, 0x03) # modem control - set RTS/DTR high
...
def main():
    ...
    while True:
        #x = sc700_fetch(SC700_I2CAdr)
        #x = sc700_fetch(SC700_I2CO)
        #print "700: {:02X}".format(x)

        slave_addr = 0x77
        y = sc700_slave_read(slave_addr, 1)
        print "700 temp: {}".format(y)
