...
int list_files(glob_t *pglob)
{
    ...
    int glob_flags = GLOB_TILDE; // look in home directory
    int i = 0;
    int rc = -1;
    ...
    while (fgets(line, MAX_LINE-1, file) != NULL) {
        ...
        // workaround for glob idiosyncracy
...
{
    debug("use_or: %d, found_count: %d, search_len: %d",
...
int scan_file(const char *filename, int use_or, int search_len, char *search_for[])
{
    ...
    // read each line of the file and search for contents
    while (fgets(line, MAX_LINE-1, file) != NULL) {
        for (i = 0; i < search_len; i++) {
            if (strcasestr(line, search_for[i]) != NULL) {
                debug("file: %s, line: %s, search: %s",
                        filename, line, search_for[i]);
                found_count++;
            }
        }
