...
#define EPSILON     0.000001    /* quantity small enough to be zero */

#define	max(A, B)		((A) > (B) ? (A) : (B))
#define min(A, B)		((A) < (B) ? (A) : (B))

/* representation of line: ax + by + c = 0
 */
typedef struct {
...
bool segments_intersect(segment s1, segment s2)
{
    ...
    return (point_in_box(p, s1.p1, s2.p2) && point_in_box(p, s2.p1, s2.p2));
