...
int do_mode(string line, int key) {
    cout << "mode : " << line;
    return 0;
}

int do_scenario(string line, int key) {
    cout << "scenario : " << line;
    return 0;
}

int do_send(string line, int key) {
    cout << "send : " << line;
    return 0;
}

int do_activate(string line, int key) {
    cout << "activate : " << line;
    return 0;
}

int do_wait(string line, int key) {
    cout << "wait : " << line;
    return 0;
}

int do_end(string line, int key) {
    cout << "end : " << line;
    return 0;
}

typedef int(*p_func) (string line, int key);
...
string strip(string token) {
    char c;
    int i = 0;
    //cout << "strip - enter: " << token << endl;
    while (i != token.size()) {
        if (token[i] == '(') break;
        ++i;
    }
    string strip(token.substr(0, i));
    return strip;
}
...
void parse(const string& file_name)
{
    ...
    while (sfp.get_next_line()) {
       ...
       for(size_t i=0;i<sfp.get_num_tokens();i++) {
          ...
          //}
          if (i == 0) {
              token = sfp.get_token(0);
              token = strip(token);
              if (my_parsers[token]) {
                  my_parsers[token](token, 7);
              }
              cout << endl;
          }
          cout << "token[" 
