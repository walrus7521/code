 ...
 *      Author: dweller
 */
// need to abstract anything schema specific: s_record
// make s_record a class to do it's own alloc and free
...
int store_result(void * pextra, int nfields, char ** arrvalues, char ** arrfieldnames) {
    ...
    for (n = 0; n < nfields; n++) {
        if (strcmp(arrfieldnames[n], "MovieTitle") == 0)
            prec->name = strdup(arrvalues[n]); /* key */
        else if (strcmp(arrfieldnames[n], "MovieCategory") == 0)
            prec->category = strdup(arrvalues[n]);
        else if (strcmp(arrfieldnames[n], "ProductionYear") == 0)
            prec->year = atoi(arrvalues[n]);
        else if (strcmp(arrfieldnames[n], "Format") == 0)
            prec->format = strdup(arrvalues[n]);
        else if (strcmp(arrfieldnames[n], "Language") == 0)
            prec->language = strdup(arrvalues[n]);
        else if (strcmp(arrfieldnames[n], "Web") == 0)
            prec->url = strdup(arrvalues[n]);
    }
