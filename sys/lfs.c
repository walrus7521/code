 ...
 * http://www.cs.columbia.edu/~jae/4118/hw/HW5a-fs.html
 *
 * mkdir mnt
 * sudo insmod ./lwnfs.ko
 * sudo mount -t lwnfs none ./mnt
 * sudo su
 * cd mnt
 * ll
 * cat counter
 ...
 * umount mnt
 *
 * Also, checkout how to mount a file as a regular file system on linux:
...
{
    printk("lfs_get_super...entered devname %s\n", devname);
...
{
    int rc;
    rc = register_filesystem(&lfs_type);
    printk("yay registered lfs - rec %d\n", rc);
	return rc;
}
 
static void __exit lfs_exit(void)
{
    printk("oh no, tearing down lfs\n");
