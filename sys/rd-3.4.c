                    ...
                    unsigned long arg)
{
    ...
    switch (cmd) {
        case HDIO_GETGEO:
            ...
            geo.heads = 4;
            geo.sectors = 16;
            printk("rd_ioctl: HDIO_GETGEO - size %lx\n", size);
...
static int get_instance(void)
{
    static int instance = 0;
    return instance++;
}

static int rd_alloc_ns(struct rd_dev *dev, unsigned nsid, unsigned long long size)
{
    struct gendisk *disk;
...
static int __init rd_init(void)
{
    ...
    for (i = 0; i < 2; i++) {
        ...
        dev->instance = get_instance();
        printk("creating disk %d\n", dev->instance);
        result = rd_alloc_ns(dev, dev->instance, dev->size);
        if (result) {
            goto error_exit;
        }
        add_disk(dev->disk);
        list_add(&dev->entry, &dev_list);
    }
...
{
    struct rd_dev *dev, *next;
    list_for_each_entry_safe(dev, next, &dev_list, entry) {
        printk("removing disk %d\n", dev->instance);
        list_del(&dev->entry);
        rd_dev_remove(dev);
        vfree(dev->data);
        kfree(dev);
